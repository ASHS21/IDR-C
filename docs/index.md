# Identity Radar Documentation

> AI-powered IAM posture management platform with Active Directory tiering, non-human identity tracking, and automated compliance.

## Getting Started
- [Quickstart](./getting-started/quickstart.md) — Get running in 5 minutes
- [Installation](./getting-started/installation.md) — Full installation guide
- [Core Concepts](./getting-started/concepts.md) — Ontology, tiering, scoring

## User Guide
- [Dashboard Overview](./user-guide/dashboard-overview.md)
- [Managing Identities](./user-guide/managing-identities.md)
- [AD Tiering](./user-guide/ad-tiering.md)
- [Violations](./user-guide/violations.md)
- [AI Analysis](./user-guide/ai-analysis.md)

## Admin Guide
- [Organization Setup](./admin-guide/organization-setup.md)
- [User Management](./admin-guide/user-management.md)
- [Integrations](./admin-guide/integrations/overview.md)
- [Policies](./admin-guide/policies.md)

## API Reference
- [Overview](./api-reference/overview.md)
- [Identities API](./api-reference/identities.md)
- [Violations API](./api-reference/violations.md)
- [Actions API](./api-reference/actions.md)

## Deployment
- [Requirements](./deployment/requirements.md)
- [Docker](./deployment/docker.md)
- [Environment Variables](./deployment/environment-variables.md)
- [Hardened Environment (EDR/XDR)](./deployment/hardened-environment.md) — Deploying alongside Defender, Carbon Black, CrowdStrike
- [SOC Change-Record Template](./deployment/soc-change-record-template.md) — Fill-in one-pager for blue-team sign-off

## Security
- [Architecture](./security/architecture.md)
- [Authentication](./security/authentication.md)
- [Authorization](./security/authorization.md)
